...
export default function App() {
    ...
    const views = {
        dashboard: <DashboardView wallets={wallets} walletBalances={walletBalances} totalBalance={totalBalance}
...
function DashboardView({
    ...
    return (<div className="container mx-auto max-w-lg p-4 pb-24">
        ...
        <div className="mb-6">
            <div className="flex justify-between items-center mb-2"><h2 className="text-xl font-bold">Daftar Dompet</h2>
                <button onClick={onOpenWalletSettings} className="text-blue-600 p-2"><EditIcon/></button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">{wallets.map(wallet => (
                <div key={wallet.id} onClick={() => onSelectWallet(wallet.id)}
                    ...
                    <div className="p-2 bg-blue-100 rounded-full"><WalletIcon/></div>
                    <div><p className="font-semibold">{wallet.name}</p><p
                        className="font-bold text-blue-600">{formatCurrency(walletBalances[wallet.id] || 0)}</p></div>
                </div>))}</div>
        </div>
...
const SearchableWalletSelector = (props) => <SearchableDropdown {...props} items={props.wallets}
